...
class WebSocketServer:
    ...
    async def _handle_connection(self, websocket):
        ...
        self.logger.bind(tag=TAG).info(f"[WEBSOCKET] NEW CONNECTION: {websocket.remote_address}")
        ...
        try:
            self.logger.bind(tag=TAG).info(f"[CONNECTION] STARTING HANDLER: session={handler.session_id}")
            await handler.handle_connection(websocket)
        finally:
            self.logger.bind(tag=TAG).info(f"[CONNECTION] CLOSED: session={handler.session_id}")
