...
class ServerPluginExecutor(ToolExecutor):
    ...
    def get_tools(self) -> Dict[str, ToolDefinition]:
        ...
        # Get functions from config - handle both old and new config structures
        config_functions = []
        try:
            # Try new config structure first
            if "Intent" in self.config and "function_call" in self.config["Intent"]:
                config_functions = self.config["Intent"]["function_call"].get("functions", [])
            # Fallback to old config structure
            elif "selected_module" in self.config and "Intent" in self.config["selected_module"]:
                config_functions = self.config["Intent"][
                    self.config["selected_module"]["Intent"]
                ].get("functions", [])
        except (KeyError, TypeError) as e:
            self.conn.logger.warning(f"Error parsing Intent functions from config: {e}")
            config_functions = []
