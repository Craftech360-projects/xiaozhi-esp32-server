...
class ASRProvider(ASRProviderBase):
    def __init__(self, config, delete_audio_file):
        ...
        # If configured as internal address (containing -internal.aliyuncs.com), use ws protocol, default is wss protocol
    ...
    async def _start_recognition(self, conn):
        ...
        logger.bind(tag=TAG).info("Start request sent, waiting for server ready...")
        ...
        try:
            while self.asr_ws and not conn.stop_event.is_set():
                try:
                    ...
                    if status != 20000000:
                        if status in [40000004, 40010004]:  # Connection timeout or client disconnect
                    ...
                    # Receiving TranscriptionStarted means server is ready to receive audio data
                    if message_name == "TranscriptionStarted":
                        self.server_ready = True
                        logger.bind(tag=TAG).info("Server ready, starting to send cached audio...")
                        # Send cached audio
                        if conn.asr_audio:
    ...
    async def _cleanup(self, conn):
        """Clean up resources"""
        logger.bind(tag=TAG).info(f"Starting ASR session cleanup | Current status: processing={self.is_processing}, server_ready={self.server_ready}")
        
        # Clear connection audio cache
        ...
        if self.asr_ws and should_stop:
            try:
                ...
                logger.bind(tag=TAG).error(f"Failed to send ASR termination request: {e}")
        
        # Status reset (after termination request is sent)
        self.is_processing = False
        self.server_ready = False
        logger.bind(tag=TAG).info("ASR status reset")
        ...
        if self.forward_task and not self.forward_task.done():
            ...
            try:
                ...
                logger.bind(tag=TAG).debug(f"forward_task cancel exception: {e}")
            finally:
                self.forward_task = None
    ...
    @langfuse_tracker.track_stt("aliyun")
