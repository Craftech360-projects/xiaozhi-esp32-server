...
class ASRProviderBase(ABC):
    ...
    async def receive_audio(self, conn, audio, audio_have_voice):
        ...
        if hasattr(conn, 'listen_start_time') and conn.listen_start_time is not None:
            ...
            # Ignore first 100ms of audio (echo period)
            if time_since_listen_start < 0.1:
                if have_voice:
                    logger.bind(tag=TAG).debug(
                        f"Ignoring potential echo audio ({time_since_listen_start:.2f}s after listen start)")
                have_voice = False
        ...
        if conn.client_voice_stop:
            ...
            if hasattr(conn, 'just_started_listening') and conn.just_started_listening:
                logger.bind(tag=TAG).debug(
                    "Skipping stale audio chunks after listen start")
                conn.just_started_listening = False
            ...
            if len(asr_audio_task) > 20:  # Increased from 15 to 20 chunks (1.2 seconds)
                logger.bind(tag=TAG).debug(
                    f"Processing {len(asr_audio_task)} audio chunks")
        ...
        try:
            ...
            if self.asr_filter and raw_text:
                ...
                should_filter, reason = self.asr_filter.should_filter(
                    raw_text, filter_context)

                if should_filter:
                    logger.bind(tag=TAG).info(
                        f"Filtered transcript: '{raw_text}' - Reason: {reason}")
    ...
    def log_audio_transcript(self, file_path: str, audio_length_seconds: float, transcript: str):
        ...
        try:
            ...
            log_dir = os.path.join(
                os.path.dirname(self.output_dir), "asr_logs")
            os.makedirs(log_dir, exist_ok=True)

            # Create both CSV and JSON log files for easy inspection
            csv_log_file = os.path.join(
                log_dir, f"asr_log_{datetime.now().strftime('%Y%m%d')}.csv")
            json_log_file = os.path.join(
                log_dir, f"asr_log_{datetime.now().strftime('%Y%m%d')}.json")

            # Write to CSV file
            csv_exists = os.path.exists(csv_log_file)
            with open(csv_log_file, 'a', newline='', encoding='utf-8') as csvfile:
                fieldnames = ['timestamp', 'filename', 'file_path',
                              'audio_length_seconds', 'transcript']
            ...
            txt_log_file = os.path.join(
                log_dir, f"asr_log_{datetime.now().strftime('%Y%m%d')}.txt")
