...
def create_instance(class_name, *args, **kwargs):
    ...
    raise ValueError(f"Unsupported TTS type: {class_name}, please check if the type in this configuration is set correctly")

class MarkdownCleaner:
    """
    Encapsulates Markdown cleaning logic: directly use MarkdownCleaner.clean_markdown(text)
    """
    ...
    def _replace_inline_dollar(m: re.Match) -> str:
        ...
        - If there are typical formula characters inside => remove the $ on both sides
    ...
    def _replace_table_block(match: re.Match) -> str:
        """
        When a complete table block is matched, this function is called back.
        """
    ...
    # The static methods replace_xxx need to be defined first so they can be correctly referenced in the list.
    REGEXES = [
