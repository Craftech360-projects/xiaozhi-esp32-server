...
def initialize_modules(
    ...
    if init_tts:
        ...
        logger.bind(tag=TAG).info(f"Component initialization: tts successful {select_tts_module}")
    ...
    if init_llm:
        ...
        logger.bind(tag=TAG).info(f"Component initialization: llm successful {select_llm_module}")
    ...
    if init_intent:
        ...
        logger.bind(tag=TAG).info(f"Component initialization: intent successful {select_intent_module}")
    ...
    if init_memory:
        ...
        logger.bind(tag=TAG).info(f"Component initialization: memory successful {select_memory_module}")
    ...
    if init_vad:
        ...
        logger.bind(tag=TAG).info(f"Component initialization: vad successful {select_vad_module}")
    
    # Initialize ASR module
    if init_asr:
        select_asr_module = config["selected_module"]["ASR"]
        modules["asr"] = initialize_asr(config)
        logger.bind(tag=TAG).info(f"Component initialization: asr successful {select_asr_module}")
