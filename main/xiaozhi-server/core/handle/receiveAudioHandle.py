...
async def startToChat(conn, text):
    conn.logger.bind(tag=TAG).info(f"[CONVERSATION] STARTED: session={conn.session_id}, text='{text[:100]}...'")
    ...
    # Filter out system messages that shouldn't go to LLM
    system_messages = [
        'voice input ready', 'voice input ready...', 'listening...', 'processing...',
        'ready to listen', 'input ready', 'ready', 'waiting', 'listening',
        'audio ready', 'audio input ready', 'microphone ready'
    ]

    # Check if the entire message matches a system message pattern
    actual_text_lower = actual_text.lower().strip()

    # Check exact matches and partial matches for system messages
    is_system_message = (
        actual_text_lower in system_messages or
        actual_text_lower.startswith('voice input ready') or
        actual_text_lower.startswith('listening') or
        actual_text_lower.startswith('ready') or
        len(actual_text.strip()) < 5  # Filter very short messages that are likely system noise
    )

    if is_system_message:
        conn.logger.bind(tag=TAG).debug(f"[SYSTEM_FILTER] Filtering out system message: '{actual_text}'")
        return

    conn.logger.bind(tag=TAG).info(f"[LLM] SENDING TO LLM: session={conn.session_id}, text='{actual_text[:100]}...'")
    await send_stt_message(conn, actual_text)
    conn.executor.submit(conn.chat, actual_text)
