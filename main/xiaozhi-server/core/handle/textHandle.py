            ...
            if msg_json["state"] == "start":
                ...
                if "text" in msg_json:
                    ...
                    is_wakeup_words = filtered_text in conn.config.get(
                        "wakeup_words")
                    # Whether wakeup word response is enabled
                    enable_greeting = conn.config.get("enable_greeting", True)

                    if is_wakeup_words and not enable_greeting:
                        # If it's a wakeup word and wakeup word response is disabled, no need to respond
                        await send_stt_message(conn, original_text)
                        await send_tts_message(conn, "stop", None)
                        conn.client_is_speaking = False
                    elif is_wakeup_words:
                        conn.just_woken_up = True
                        # Report text-only data (reuse ASR reporting functionality, but don't provide audio data)
