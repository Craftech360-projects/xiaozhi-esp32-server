...
class ConnectionHandler:
    def __init__(
        ...
        self.langfuse = langfuse_config.get_client()
        if self.langfuse:
            self.logger.bind(tag=TAG).info(
                "Langfuse client is available for this connection.")
        else:
            self.logger.bind(tag=TAG).warning(
                "Langfuse client is NOT available for this connection.")
        self.trace = None
        ...
        # Store last 10 chunks (600ms) before voice detection
        self.audio_pre_buffer = deque(maxlen=10)
    ...
    async def handle_connection(self, ws):
        ...
        self.logger.bind(tag=TAG).critical("!!! CONNECTION HANDLER STARTED. WAITING FOR MESSAGES... !!!")
        self.logger.bind(tag=TAG).critical("!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!")
        try:
            # Get and validate headers
            self.headers = dict(ws.request.headers)
            ...
            self.logger.bind(tag=TAG).info(f"Welcome message sent: {self.welcome_msg['type']}")

            try:
                async for message in self.websocket:
                    self.logger.bind(tag=TAG).debug(
                        f"[WEBSOCKET] RECV: type={type(message)}, size={len(message)}")
                    await self._route_message(message)
            except websockets.exceptions.ConnectionClosed:
                self.logger.bind(tag=TAG).info("Client disconnected")
    ...
    async def _save_and_close(self, ws):
        ...
        try:
            if self.memory:
                ...
                if memory_type == "nomem":
                    self.logger.bind(tag=TAG).info(
                        "Memory saving DISABLED (using nomem)")
                elif memory_type == "mem0ai":
                    self.logger.bind(tag=TAG).info(
                        "Saving memory to MEM0 CLOUD")
                elif memory_type == "mem_local_short":
                    self.logger.bind(tag=TAG).info(
                        "Saving memory to LOCAL STORAGE (mem_local_short)")
                else:
                    self.logger.bind(tag=TAG).info(
                        f"Saving memory using: {memory_type}")

                # Use a thread pool to save memory asynchronously
                def save_memory_task():
                    try:
                        self.logger.bind(tag=TAG).info(
                            f"[MEM0-THREAD] Starting memory save task for {memory_type}")
                        self.logger.bind(tag=TAG).info(
                            f"[MEM0-THREAD] Dialogue messages count: {len(self.dialogue.dialogue) if self.dialogue and self.dialogue.dialogue else 0}")
                        ...
                        self.logger.bind(tag=TAG).info(
                            f"[MEM0-THREAD] Calling memory.save_memory()")
                        result = loop.run_until_complete(
                            self.memory.save_memory(self.dialogue.dialogue)
                        )
                        ...
                        if result:
                            self.logger.bind(tag=TAG).info(
                                f"[MEM0-THREAD] Memory saved successfully using {memory_type}. Result: {result}")
                        else:
                            self.logger.bind(tag=TAG).warning(
                                f"[MEM0-THREAD] Memory save returned None/False for {memory_type}")
                    except Exception as e:
                        self.logger.bind(tag=TAG).error(
                            f"[MEM0-THREAD] Failed to save memory: {e}")
                        self.logger.bind(tag=TAG).error(
                ...
                # Start a thread to save memory, do not wait for completion
                self.logger.bind(tag=TAG).info(
                    f"[MEM0-THREAD] Starting new thread for memory save")
                threading.Thread(target=save_memory_task, daemon=True).start()
        except Exception as e:
            self.logger.bind(tag=TAG).error(f"Failed to save memory: {e}")
        finally:
    ...
    def _initialize_private_config(self):
        ...
        try:
            ...
            if "Memory" in private_config and "Memory_mem0ai" in private_config.get("Memory", {}):
                ...
                # DEBUG: Print the full API key to verify what's being received
                self.logger.bind(tag=TAG).info(
                    f"[DEBUG] Raw mem0 API key received: '{api_key}'")
                ...
                if api_key == "***":
                    ...
                    masked_key = f"{api_key[:4]}...{api_key[-4:]}" if len(
                        api_key) > 8 else "****"
                    self.logger.bind(tag=TAG).debug(
                        f"Mem0 API key received: {masked_key} (length: {len(api_key)})")
                else:
                    self.logger.bind(tag=TAG).warning(
                        "Mem0 API key is empty in differential configuration")
    ...
    def _initialize_memory(self):
        ...
        if memory_type == "nomem":
            self.logger.bind(tag=TAG).info(
                "Memory module initialized: DISABLED (nomem)")
            return
        # Use mem_local_short mode
        elif memory_type == "mem_local_short":
            self.logger.bind(tag=TAG).info(
                "Memory module initialized: LOCAL STORAGE (mem_local_short)")
            # ALWAYS use the main LLM for memory operations
            # This ensures consistency and avoids configuration issues
            self.memory.set_llm(self.llm)
            self.logger.bind(tag=TAG).info(
            ...
            self.logger.bind(tag=TAG).info(
                "Memory module initialized: MEM0 CLOUD (mem0ai)")
        else:
            self.logger.bind(tag=TAG).info(
                f"Memory module initialized: {memory_type}")
    ...
    def chat(self, query, tool_call=False, depth=0):
        self.logger.bind(tag=TAG).info(
            f"[LLM] CHAT STARTED: session={self.session_id}, query='{query[:100]}...', tool_call={tool_call}, depth={depth}")
        self.llm_finish_task = False
        ...
        trace = None
        if self.langfuse and depth == 0:  # Create trace only for the top-level call
            try:
                # Using correct Langfuse v3.3+ API for conversation tracking
                event = self.langfuse.create_event(
                    name="voice-conversation-turn",
                    metadata={
                ...
                self.logger.bind(tag=TAG).info(
                    f"Langfuse conversation event created successfully")
                trace = event  # Use event as trace placeholder
            except Exception as e:
                self.logger.bind(tag=TAG).error(
                    f"Failed to create langfuse event: {e}")
                trace = None
        ...
        functions = None
        self.logger.bind(tag=TAG).info(f"Function handler check - intent_type: {self.intent_type}, has_func_handler: {hasattr(self, 'func_handler')}")
        if self.intent_type == "function_call" and hasattr(self, "func_handler"):
            functions = self.func_handler.get_functions()
            self.logger.bind(tag=TAG).info(f"Retrieved {len(functions) if functions else 0} functions from handler")
        response_message = []
        ...
        if trace:
            span = trace.span(
                name="llm-generation",
                input=self.dialogue.get_llm_dialogue(),
                metadata={
                    "tool_call": tool_call,
                    "depth": depth,
                },
        ...
        try:
            memory_str = None
            if self.memory is not None:
                self.logger.bind(tag=TAG).info(f"Querying memory for: '{query[:50]}...'")
                try:
                    future = asyncio.run_coroutine_threadsafe(
                        self.memory.query_memory(query), self.loop
                    )
                    # Add timeout to prevent hanging
                    memory_str = future.result(timeout=10)  # 10 second timeout
                    self.logger.bind(tag=TAG).info(f"Memory query completed, result length: {len(memory_str) if memory_str else 0}")
                except Exception as memory_error:
                    self.logger.bind(tag=TAG).error(f"Memory query failed: {memory_error}")
                    memory_str = None  # Continue without memory
            else:
                self.logger.bind(tag=TAG).info("No memory module configured")

            # Debug logging before LLM calls
            dialogue_data = self.dialogue.get_llm_dialogue_with_memory(
                memory_str, self.config.get("voiceprint", {})
            )
            self.logger.bind(tag=TAG).info(f"About to call LLM - intent_type: {self.intent_type}, functions: {functions is not None}, dialogue_length: {len(dialogue_data) if dialogue_data else 0}")

            if self.intent_type == "function_call" and functions is not None:
                self.logger.bind(tag=TAG).info(f"Calling response_with_functions with {len(functions)} functions")
                llm_responses = self.llm.response_with_functions(
                    self.session_id,
                    dialogue_data,
                    functions=functions,
                ...
                self.logger.bind(tag=TAG).info(f"Calling regular response method")
                llm_responses = self.llm.response(
                    self.session_id,
                    dialogue_data,
                )
            ...
            if trace:
                trace.update(level='ERROR', status_message=f"LLM Error: {e}")
                self.langfuse.flush()
            return None
        ...
        if len(response_message) > 0:
            ...
            self.dialogue.put(Message(role="assistant", content=text_buff))
            if span:
                span.end(output=text_buff)
            if trace:
                trace.update(output=text_buff)
        ...
        if depth == 0:
            ...
            if trace:  # Final flush for the top-level trace
                self.langfuse.flush()
    ...
    def reset_vad_states(self):
        ...
        if hasattr(self, 'asr_audio'):
            self.asr_audio.clear()
        # Reset VAD recording start time (no timeout logic needed)
        ...
        if hasattr(self, 'asr_audio_queue'):
            ...
            if cleared_count > 0:
                self.logger.bind(tag=TAG).debug(
                    f"Cleared {cleared_count} audio chunks from ASR queue")
        self.logger.bind(tag=TAG).info("VAD states reset.")
