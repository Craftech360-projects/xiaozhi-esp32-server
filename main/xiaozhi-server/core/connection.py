...
class ConnectionHandler:
    def __init__(
        ...
        # ASR provider reference for VAD streaming
    ...
    async def _route_message(self, message):
        ...
        if isinstance(message, str):
            self.logger.bind(tag=TAG).debug(f"Received text message: {message[:100]}...")
            await handleTextMessage(self, message)
        elif isinstance(message, bytes):
            self.logger.bind(tag=TAG).debug(f"Received audio message: {len(message)} bytes")
            if self.vad is None:
                self.logger.bind(tag=TAG).error("VAD is None, cannot process audio")
                return
            if self.asr is None:
    ...
    async def _process_streaming_audio(self, audio_packet):
        ...
        This method is called for each incoming audio packet to enable
        real-time streaming to ASR providers like Chirp 2.
        """
        try:
            self.logger.bind(tag=TAG).debug(f"Processing streaming audio packet: {len(audio_packet)} bytes")
            
            # Call VAD directly for this audio packet
            # This will handle streaming to ASR if voice is detected
            have_voice = await self.vad.is_vad(self, audio_packet)
            
            # Update activity timestamp
            if have_voice:
                self.last_activity_time = time.time() * 1000
                self.logger.bind(tag=TAG).debug(f"Voice detected in streaming audio packet")
            
        except Exception as e:
            self.logger.bind(tag=TAG).error(f"Error processing streaming audio: {e}")
            import traceback
            self.logger.bind(tag=TAG).debug(f"Streaming audio error traceback: {traceback.format_exc()}")
    ...
    def _initialize_components(self):
        try:
            ...
            # Set ASR provider for VAD streaming
            self.asr_provider = self.asr

            # Initialize voiceprint recognition
            self._initialize_voiceprint()

            # Open speech recognition channel
            self.logger.bind(tag=TAG).info("Opening ASR audio channels...")
            asyncio.run_coroutine_threadsafe(
                self.asr.open_audio_channels(self), self.loop
            )
            self.logger.bind(tag=TAG).info("ASR audio channels opened")
            if self.tts is None:
    ...
    def _initialize_private_config(self):
        ...
        if modules.get("asr", None) is not None:
            ...
            # Update ASR provider reference for VAD streaming
            self.asr_provider = self.asr
