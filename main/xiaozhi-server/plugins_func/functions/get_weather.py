...
GET_WEATHER_FUNCTION_DESC = {
    ...
    "function": {
        ...
        "description": (
            ...
            "If user mentions a province, default to using the provincial capital. If user mentions a place name that's not a province or city, default to using the provincial capital of that province. "
            "If user doesn't specify location, saying 'how's the weather', 'how's the weather today', location parameter is empty"
        ),
        "parameters": {
            "type": "object",
            "properties": {
...
def get_weather(conn, location: str = None, lang: str = "zh_CN"):
    ...
    if not location:
        ...
        if client_ip:
            # Dynamically parse city information corresponding to IP
            ip_info = get_ip_info(client_ip, logger)
            location = ip_info.get("city") if ip_info and "city" in ip_info else None
        else:
            # If IP parsing fails or no IP, use default location
    ...
    if not city_info:
        return ActionResponse(
            Action.REQLLM, 
            f"Related city not found: {location}, please confirm if location is correct", 
            None
        )
