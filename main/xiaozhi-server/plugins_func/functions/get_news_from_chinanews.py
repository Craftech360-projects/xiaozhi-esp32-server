...
GET_NEWS_FROM_CHINANEWS_FUNCTION_DESC = {
    ...
    "function": {
        ...
        "description": (
            ...
            "Users can specify news category, such as social news, tech news, international news, etc. "
            "If not specified, default to social news. "
...
def fetch_news_detail(url):
    ...
    try:
        ...
        # Try to extract body content (selectors need to be adjusted based on actual website structure)
...
def map_category(category_text):
    ...
    if not category_text:
        return None
    # Category mapping dictionary, currently supports society, international, finance news. For more types, see config file
    category_map = {
        # Society news
        "社会": "society_rss_url",
    ...
    # Return mapping result, return original input if no match
...
def get_news_from_chinanews(
    ...
    try:
        ...
        # If category provided, try to get corresponding URL from config
        ...
        if not news_items:
            return ActionResponse(
                Action.REQLLM, 
                "Sorry, unable to get news information, please try again later.", 
                None
            )
        ...
        news_report = (
            ...
            f"just read the news directly, no need for extra unnecessary content. "
            f"If user asks for more details, tell user they can say 'please introduce this news in detail' to get more content)"
        )
