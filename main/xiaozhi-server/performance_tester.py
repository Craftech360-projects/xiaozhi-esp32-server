...
class AsyncPerformanceTester:
            ...
            try:
                ...
                async with session.get(f"{base_url}/api/version") as response:
                    if response.status != 200:
                        print(f"🚫 Ollama service is not running or inaccessible: {base_url}")
                        return False
    ...
    async def _test_tts(self, tts_name: str, config: Dict) -> Dict:
        ...
        try:
            ...
            if any(
                ...
                print(f"⏭️ TTS {tts_name} access_token/api_key not configured, skipped")
    ...
    async def _test_stt(self, stt_name: str, config: Dict) -> Dict:
        ...
        try:
            ...
            if any(
                ...
                print(f"⏭️ STT {stt_name} access_token/api_key not configured, skipped")
    ...
    async def _test_llm(self, llm_name: str, config: Dict) -> Dict:
        ...
        try:
            ...
            if not valid_results:
                print(f"⚠️ {llm_name} no valid data, configuration may be incorrect")
                return {"name": llm_name, "type": "llm", "errors": 1}
            ...
            if len(filtered_times) < len(test_sentences) * 0.5:
                print(f"⚠️ {llm_name} insufficient valid data, network may be unstable")
                return {"name": llm_name, "type": "llm", "errors": 1}
    ...
    async def _test_single_sentence(self, llm_name: str, llm, sentence: str) -> Dict:
        """Test performance of single sentence"""
        try:
            ...
            if first_token_time is None:
                first_token_time = (
                    response_time  # If first token not detected, use total response time
                )
    ...
    def _print_results(self):
        ...
        if llm_table:
            ...
            print(
                tabulate(
                    llm_table,
                    headers=["Model Name", "First Token Time", "Total Time", "Stability"],
                    tablefmt="github",
        ...
        if self.results["combinations"]:
            print("\nRecommended Configuration Combinations (lower score is better):\n")
            combo_table = []
            ...
            for llm_name, config in self.config.get("LLM", {}).items():
                ...
                if llm_name == "CozeLLM":
                    if any(x in config.get("bot_id", "") for x in ["your"]) or any(
                        ...
                        print(f"⏭️ LLM {llm_name} bot_id/user_id not configured, skipped")
                        continue
                ...
                # Create independent task for each sentence
            ...
            if self.config.get("ASR") is not None:
                for stt_name, config in self.config.get("ASR", {}).items():
                    ...
                    if any(
                        ...
                        print(f"⏭️ ASR {stt_name} access_token/api_key not configured, skipped")
                        continue

                    print(f"🎵 Adding ASR test task: {stt_name}")
                    all_tasks.append(self._test_stt(stt_name, config))
        else:
            print(f"\n⚠️ No audio files found in {self.wav_root} directory, STT test tasks skipped")
