...
SERVER_IP = "192.168.1.77"  # !!! UPDATE with your server's local IP address !!!
OTA_PORT = 8003
MQTT_BROKER_HOST = "192.168.1.77"  # MQTT gateway IP
...
class TestClient:
        ...
        try:
            ...
            # Handle TTS stop signal (start recording for next user input)
                ...
                if self.total_packets_received > 0:
                    ...
                    logger.warning(
                        "⚠️ No audio packets received during TTS. Starting voice input...")
                    # Start voice listening instead of retry
                    threading.Timer(2.0, self.start_voice_listening).start()
    ...
    def start_voice_listening(self):
        """Start continuous voice listening instead of retry attempts."""
        if self.session_active and not self.tts_active:
            self.voice_sessions += 1
            logger.info(f"🎤 Voice session #{self.voice_sessions}: Ready for input. Please speak into your microphone...")

            # Clear stop event and set start event to begin recording
            stop_recording_event.clear()
            start_recording_event.set()

            # Send listen message without retry text
            listen_payload = {
                "type": "listen",
            ...
            if self.mqtt_client:
                ...
                logger.info("🎤 Voice detection enabled. Speak now!")
        else:
            logger.warning("⚠️ Cannot start voice listening - session inactive or TTS active")
    ...
    def trigger_conversation(self):
        ...
        listen_payload = {
            "type": "listen", "session_id": udp_session_details["session_id"], "state": "detect", "text": "Hello! I'm ready to chat with you."}
            ...
            while not stop_threads.is_set() and self.session_active:
                ...
                if self.tts_active and time.time() - self.last_audio_received > TTS_TIMEOUT_SECONDS:
                    ...
                    if timeout_count >= 3:
                        ...
                        logger.info(
                            "🔄 Attempting to recover by starting voice listening...")
