...
class TestClient:
    ...
    def get_ota_config(self) -> bool:
        ...
        logger.info(
            f"▶️ STEP 1: Requesting OTA config from http://{SERVER_IP}:{OTA_PORT}/xiaozhi/ota/")
        try:
            ...
            response = requests.post(
                f"http://{SERVER_IP}:{OTA_PORT}/xiaozhi/ota/", headers=headers, json=data, timeout=5)
        ...
        while not stop_threads.is_set() and self.session_active:
            try:
                ...
                if data and len(data) > 16:
                    ...
                    pcm_payload = decoder.decode(
                        opus_payload, frame_size_samples)
