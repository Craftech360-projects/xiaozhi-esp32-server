...
def check_config_file():
    ...
    if config_file_valid:
        return
    """
   Simplified configuration check, only prompts users about configuration file usage
   """
    custom_config_file = get_project_dir() + "data/." + default_config_file
    if not os.path.exists(custom_config_file):
        raise FileNotFoundError(
            "Cannot find data/.config.yaml file, please follow the tutorial to confirm if this configuration file exists"
        )
