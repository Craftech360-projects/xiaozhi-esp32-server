...
class EdgeTTS(tts.TTS if LIVEKIT_AVAILABLE else object):
    ...
    def __init__(
        ...
        volume: str = "+0%",
        pitch: str = "+0Hz",
        sample_rate: int = 24000,  # Optimized for EdgeTTS
        channels: int = 1
    ):
...
class EdgeTTSChunkedStream(tts.ChunkedStream if LIVEKIT_AVAILABLE else object):
    ...
    async def _run(self, output_emitter: tts.AudioEmitter) -> None:
        ...
        try:
            ...
            # Ultra-aggressive streaming for maximum multi-client performance
            chunk_count = 0
            total_bytes = 0
            ...
            async for chunk in communicate.stream():
                if chunk["type"] == "audio":
                    ...
                    # Push immediately with zero buffering for ultra-low latency
                    output_emitter.push(chunk["data"])
                    
                    # Yield control every few chunks to prevent blocking other clients
                    if chunk_count % 3 == 0:
                        await asyncio.sleep(0)  # Yield to event loop
            
            logger.info(f"🎤 EdgeTTS: Streamed {chunk_count} chunks, {total_bytes} bytes")
