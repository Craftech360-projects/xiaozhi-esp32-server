...
class LiveKitMCPExecutor:
    ...
    async def get_battery_status(self) -> str:
        ...
        try:
            ...
            # Parse the response
            # The response structure is: {"result": {"jsonrpc": "2.0", "result": {"content": [{"type": "text", "text": "{json_string}"}]}}}
            result = response.get("result", {})
            # Handle double-nested result structure from gateway
            inner_result = result.get("result", result)  # Try inner result, fallback to outer
