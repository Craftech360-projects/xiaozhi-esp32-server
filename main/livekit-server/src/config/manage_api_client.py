...
class ManageApiClient:
    ...
    def _init_client(cls, config):
        ...
        cls.config = config.get("manager_api")

        if not cls.config:
            raise Exception("manager_api configuration not found in config")

        if not cls.config.get("url"):
            raise Exception("manager_api url configuration missing")

        if not cls.config.get("secret"):
            raise Exception("manager_api secret configuration missing")

        if "你" in cls.config.get("secret"):
            raise Exception("Please configure the manager_api secret first")
        ...
        cls._client = httpx.Client(
            ...
            # Follow redirects to handle session-based auth
    ...
    def _request(cls, method: str, endpoint: str, **kwargs) -> Dict:
        ...
        print(f"🔍 Making request to: {cls.config.get('url')}/{endpoint}")
        ...
        if result.get("code") == 10041:
            ...
            raise Exception(f"Authentication failed: {result.get('msg', 'Unauthorized')}")
...
def report(
    mac_address: str, session_id: str, chat_type: int, content: str, report_time
) -> Optional[Dict]:
    """Business method example with circuit breaker"""
    if not content or not ManageApiClient._instance:
        print(f"Chat history report skipped: content={bool(content)}, instance={bool(ManageApiClient._instance)}")
        return None
    try:
        chat_data = {
            "macAddress": mac_address,
            "sessionId": session_id,
            "chatType": int(chat_type),  # Ensure it's an integer
            "content": content,
            "reportTime": report_time,
        }
        print(f"🔄 Sending chat history to manager-api: {chat_data}")
        print(f"🔑 Using secret: {ManageApiClient._secret[:10]}...")
        ...
        print(f"✅ Chat history sent successfully: {result}")
        return result
    except Exception as e:
        print(f"❌ Chat history reporting failed: {e}")
