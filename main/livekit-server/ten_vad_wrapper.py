...
class TENVADStream(vad.VADStream):
                ...
                while len(self._audio_buffer) >= self._hop_size:
                    ...
                    if is_speech:
                        if not self._speech_started:
                            ...
                            if self._speech_duration >= self._min_speech_duration:
                                self._speech_started = True

                                # Add prefix padding
                                self._speech_buffer.extend(self._prefix_buffer)
                                ...
                                logger.debug("[TEN VAD] START_OF_SPEECH detected")
                        ...
                        if self._speech_started:
                            ...
                            # Accumulate speech duration for threshold check
                            self._speech_duration += frame_duration
    ...
    def _resample_audio(self, audio: np.ndarray, from_rate: int, to_rate: int) -> np.ndarray:
        ...
        try:
            ...
            logger.debug(f"[TEN VAD] Resampled audio from {from_rate}Hz to {to_rate}Hz using scipy (length: {len(audio)} -> {len(resampled)})")
            return resampled
        except ImportError:
            ...
            logger.debug(f"[TEN VAD] Resampled audio from {from_rate}Hz to {to_rate}Hz using linear interpolation (length: {len(audio)} -> {len(resampled)})")
            return resampled
