 ...
 * Handles application layer logic processing
 */
...
class MQTTServer {
  constructor() {
    ...
    this.keepAliveCheckInterval = 1000; // Default check every 1 second
  ...
  start() {
    ...
    // Start global keep-alive check timer
    this.setupKeepAliveTimer();
  }

  /**
   * Setup global keep-alive check timer
   */
  setupKeepAliveTimer() {
    ...
    this.keepAliveTimer = setInterval(() => {
      // Check keep-alive status of all connections
  ...
  addConnection(connection) {
    // Check if connection with same clientId already exists
  ...
  async stop() {
    ...
    if (this.connections.size > 0) {
      ...
      debug("Waiting for connections to close completed");
