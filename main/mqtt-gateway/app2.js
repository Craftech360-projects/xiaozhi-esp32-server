...
const INCOMING_FRAME_DURATION_MS = 60;  // 60ms frames for incoming (ESP32 → LiveKit) - FIXED: was 20ms, firmware uses 60ms
...
// ========================================
// Create a single shared worker pool for all connections
// This prevents resource waste when multiple ESP32 clients connect
// Each connection should share workers instead of creating separate pools
const globalWorkerPool = new WorkerPoolManager(4); // Shared across all connections
console.log(`✅ [GLOBAL] Shared worker pool initialized (${4} workers for all connections)`);

// Initialize global worker pool with encoder/decoder settings
globalWorkerPool.initializeWorker('init_encoder', {
  sampleRate: OUTGOING_SAMPLE_RATE,
  channels: CHANNELS
}).then(() => {
  console.log(`✅ [GLOBAL] Shared workers encoder ready (${OUTGOING_SAMPLE_RATE}Hz)`);
}).catch(err => {
  console.error(`❌ [GLOBAL] Shared worker encoder init failed:`, err.message);
});

globalWorkerPool.initializeWorker('init_decoder', {
  sampleRate: INCOMING_SAMPLE_RATE,
  channels: CHANNELS
}).then(() => {
  console.log(`✅ [GLOBAL] Shared workers decoder ready (${INCOMING_SAMPLE_RATE}Hz)`);
}).catch(err => {
  console.error(`❌ [GLOBAL] Shared worker decoder init failed:`, err.message);
});

// Start periodic metrics logging for shared pool
globalWorkerPool.startMetricsLogging(30);
...
class LiveKitBridge extends Emitter {
  constructor(connection, protocolVersion, macAddress, uuid, userData) {
    ...
    this.room = null;
    // Create enhanced AudioSource with LiveKit's capabilities
    // Latest API: AudioSource(sampleRate, numChannels)
    this.audioSource = new AudioSource(16000, 1);

    // Initialize LiveKit audio enhancement pipeline
    this.initializeLiveKitAudioPipeline();
    ...
    // PHASE 2: Use GLOBAL SHARED Worker Pool instead of creating per-connection pool
    // This prevents resource waste when multiple ESP32 clients connect simultaneously
    // Each connection shares the same worker pool, reducing memory and CPU overhead
    this.workerPool = globalWorkerPool; // Use shared global worker pool
    console.log(`✅ [PHASE-2] Using shared worker pool for ${this.macAddress}`);
    ...
    // JITTER BUFFER DISABLED: Process frames immediately in arrival order
    // This eliminates timing variations and jitter in saved audio files
  ...
  /**
   * SIMPLIFIED: Initialize minimal audio processing
   * No complex enhancement pipeline - just basic setup
   */
  async initializeLiveKitAudioPipeline() {
    try {
      // Simple audio statistics for monitoring
      this.audioStats = {
        totalFrames: 0,
        opusFrames: 0,
        pcmFrames: 0,
      ...
      console.log("✅ [AUDIO] Simplified audio pipeline initialized");
      console.log("   🎵 Sample rate: 16kHz mono");
      console.log("   🔧 Processing: Minimal (clipping prevention only)");

    } catch (error) {
      console.error("❌ [AUDIO] Failed to initialize audio pipeline:", error.message);
    }
  }
  ...
  async connect(audio_params, features, roomService) {
    ...
    return new Promise(async (resolve, reject) => {
      try {
        ...
        // Latest API: Track publishing with updated method
        const {
          LocalAudioTrack,
          TrackPublishOptions,
          TrackSource,
  ...
  async sendAudio(opusData, timestamp) {
    ...
    try {
      // JITTER BUFFER DISABLED: Process frames immediately in arrival order
      // This eliminates timing variations and jitter in saved audio files
      // Frames are processed sequentially as they arrive from UDP
      await this.processAudioFrame(opusData, timestamp);

    } catch (error) {
      console.error(`❌ [AUDIO] Error in sendAudio for ${this.macAddress}: ${error.message}`);
    }
  }


  async processAudioFrame(opusData, timestamp) {
    try {
      ...
      if (isOpus) {
        this.audioStats.opusFrames++;
      } else {
        this.audioStats.pcmFrames++;
      }

      // Simplified logging every 200 frames (less verbose)
      if (this.audioFrameCount % 200 === 0) {
        const now = Date.now();
        const timeDiff = (now - this.audioStats.lastLogTime) / 1000;
        const fps = 200 / timeDiff;

        console.log(`📊 [AUDIO] Frame ${this.audioFrameCount}: ${fps.toFixed(1)} fps, Opus: ${this.audioStats.opusFrames}, PCM: ${this.audioStats.pcmFrames}, Errors: ${this.audioStats.errorFrames}`);
      ...
      if (isOpus) {
        ...
        try {
          ...
          if (this.audioFrameCount % 50 === 0) {
            console.log(`✅ [WORKER DECODE] Frame ${this.audioFrameCount}: Decoded ${opusData.length}B Opus → ${pcmBuffer.length}B PCM`);
          }

          if (pcmBuffer && pcmBuffer.length > 0) {
            // Debug: Analyze PCM data quality (only for first few frames)
            if (this.audioFrameCount <= 5) {
              this.debugAnalyzePcmData(pcmBuffer, 'OPUS_DECODED');
            }

            // Process audio with LiveKit enhancement pipeline
            const processedBuffer = await this.processAudioWithLiveKit(pcmBuffer);

            // Save processed audio to WAV file
            this.writeToWavFile(processedBuffer);

            // Convert processed Buffer to Int16Array
            const samples = new Int16Array(
              processedBuffer.buffer,
              processedBuffer.byteOffset,
              processedBuffer.length / 2
            );
            // Latest API: AudioFrame(data, sampleRate, numChannels, samplesPerChannel)
            ...
            this.safeCaptureFrame(frame).catch(err => {
              console.error(`❌ [LIVEKIT-AUDIO] Unhandled error in safeCaptureFrame: ${err.message}`);
            });
          }
        } catch (err) {
          console.error(`❌ [WORKER] Decode error frame ${this.audioFrameCount}: ${err.message}`);
          ...
          console.log(`⚠️ [FALLBACK] Frame ${this.audioFrameCount}: Treating as PCM instead`);

          // Process fallback PCM data with LiveKit enhancement
          const processedBuffer = await this.processAudioWithLiveKit(opusData);
          ...
          const frame = new AudioFrame(samples, 16000, 1, samples.length);
          this.safeCaptureFrame(frame).catch(err => {
            console.error(`❌ [LIVEKIT-AUDIO] PCM fallback failed: ${err.message}`);
          });
        }
      } else {
        // Treat as PCM directly
        if (this.audioFrameCount % 50 === 0) {
          console.log(`🎤 [PCM-DIRECT] Frame ${this.audioFrameCount}: Treating ${opusData.length}B as raw PCM`);
        }
        ...
        // Process raw PCM audio with LiveKit enhancement pipeline
        const processedBuffer = await this.processAudioWithLiveKit(opusData);
      ...
      console.error(`❌ [AUDIO] Error in sendAudio frame ${this.audioFrameCount}: ${error.message}`);
    }
  }
  ...
  async safeCaptureFrame(frame) {
    try {
      ...
      // Latest API: Attempt to capture the frame with enhanced error handling
      await this.audioSource.captureFrame(frame);
    } catch (error) {
      console.error(`❌ [AUDIO] Failed to capture frame: ${error.message}`);

      // Enhanced error handling for latest LiveKit version
      ...
      } else if (error.message.includes('AudioSource')) {
        console.warn(`⚠️ [AUDIO] AudioSource error - may need reinitialization`);
      } else if (error.message.includes('Frame')) {
        console.warn(`⚠️ [AUDIO] Frame format error - check audio data format`);
      }
    }
  }
  ...
  checkOpusFormat(data) {
    ...
    // ✅ RELAXED: More permissive Opus validation for ESP32 compatibility
    // ESP32 sends variable-size OPUS frames at 16kHz mono
    const MIN_OPUS_SIZE = 3;    // Reduced minimum - ESP32 may send small frames
    const MAX_OPUS_SIZE = 400;  // Increased maximum for variable bitrates
    ...
    // ✅ MORE PERMISSIVE: Accept wider range of Opus configs
    // Include all valid Opus configurations (0-31)
    ...
    // ✅ RELAXED: Less strict data pattern checking
    let zeroCount = 0;
    const sampleSize = Math.min(8, data.length); // Check fewer bytes
    ...
    // Only reject if ALL bytes are zero (completely invalid)
    const reasonableData = zeroRatio < 1.0;

    const isValidOpus = validConfig && validStereo && validFrameCount && reasonableData;

    // ✅ DEBUG: Log rejected packets for analysis
  ...
  /**
   * SIMPLIFIED: Minimal audio processing for maximum reliability
   * Only applies essential clipping prevention
   */
  async processAudioWithLiveKit(pcmBuffer) {
    try {
      // NO GAIN BOOST: Return raw PCM to avoid amplifying noise
      // This matches xiaozhi-server behavior for clean audio
      return pcmBuffer;

    } catch (error) {
      console.error(`❌ [AUDIO-PROCESS] Minimal processing error: ${error.message}`);
      return pcmBuffer; // Return original if processing fails
    }
  }
  ...
  /**
   * Fallback basic audio processing when LiveKit enhancement is unavailable
   */
  applyBasicProcessing(pcmBuffer) {
    try {
      // NO GAIN BOOST: Return raw PCM to avoid amplifying noise
      return pcmBuffer;
    } catch (error) {
      console.error(`❌ [BASIC-PROCESS] Basic processing error: ${error.message}`);
      return pcmBuffer; // Return original if all processing fails
    }
  }
  ...
  writeToWavFile(pcmData) {
    if (this.currentWavWriter && pcmData && pcmData.length > 0) {
      try {
        // Write raw PCM data without aggressive processing to preserve quality
        this.currentWavWriter.writeAudio(pcmData);
      } catch (error) {
  ...
  applyNoiseReduction(pcmData) {
    ...
    try {
      ...
      // Only apply minimal processing to avoid distortion
      // Step 1: Basic clipping prevention only
      const processed = this.preventClipping(samples);
  ...
  // MODERATE FIXED GAIN - Simple volume boost without logging
  applyModerateGain(samples, gain = 3.0) {
    const boosted = new Int16Array(samples.length);
    for (let i = 0; i < samples.length; i++) {
      boosted[i] = Math.round(samples[i] * gain);
    }
    return boosted;
  }

  // UNIVERSAL MAXIMUM VOLUME BOOST - FOR ALL AUDIO (DEPRECATED - kept for reference)
  applyKidVoiceBoost(samples) {
    const boosted = new Int16Array(samples.length);

    // Calculate current peak to determine boost factor
    let peak = 0;
    for (let i = 0; i < samples.length; i++) {
      peak = Math.max(peak, Math.abs(samples[i]));
    }

    // AGGRESSIVE UNIVERSAL BOOST - MAXIMUM VOLUME FOR ALL AUDIO
    let boostFactor = 5.0; // 500% boost by default - MAXIMUM POWER!

    // Even more aggressive boost for quiet audio
    if (peak < 3000) {
      boostFactor = 8.0; // 800% boost for very quiet audio
    } else if (peak < 8000) {
      boostFactor = 6.0; // 600% boost for quiet audio
    } else if (peak < 15000) {
      boostFactor = 4.0; // 400% boost for moderate audio
    } else if (peak < 25000) {
      boostFactor = 2.5; // 250% boost for louder audio
    } else {
      boostFactor = 1.5; // 150% boost for very loud audio (minimal to prevent distortion)
    }

    console.log(`🚀 [MAXIMUM-VOLUME-BOOST] Peak: ${peak}, Boost: ${boostFactor}x (${(boostFactor * 100).toFixed(0)}%)`);

    // Apply the massive boost
    for (let i = 0; i < samples.length; i++) {
      boosted[i] = Math.round(samples[i] * boostFactor);
    }

    return boosted;
  }

  // Prevent clipping with minimal limiting - MAXIMUM VOLUME FOR KID VOICES
  preventClipping(samples) {
    const limited = new Int16Array(samples.length);
    // Use 100% of available range for maximum kid voice volume
    const maxSafe = 32700; // 99.8% of max (32767) - minimal headroom
    const minSafe = -32700;

    for (let i = 0; i < samples.length; i++) {
      let sample = samples[i];

      // Minimal soft limiting - preserve maximum volume
      if (sample > maxSafe) {
        const excess = sample - maxSafe;
        const compressed = excess * 0.9; // Minimal compression (10% only)
  ...
  debugAnalyzePcmData(pcmData, source) {
    ...
    if (this.debugPacketCount > 5) return; // Only analyze first 5 packets
  ...
  async close() {
    ...
    // Jitter buffer cleanup removed - direct processing enabled

    // Simple cleanup
    if (this.audioStats) {
      console.log(`📊 [CLEANUP] Audio stats - Total: ${this.audioStats.totalFrames}, Opus: ${this.audioStats.opusFrames}, PCM: ${this.audioStats.pcmFrames}, Errors: ${this.audioStats.errorFrames}`);
...
/**
 * MQTT connection class for EMQX broker connections
 * Handles device connections through EMQX broker
 */
class VirtualMQTTConnection {
  ...
  async parseOtherMessage(json) {
    ...
    if (json.type === "listen" || json.type === "start_ptt" || json.type === "end_ptt" || json.type === "start_greeting") {
      try {
        let isPttStart = false;
        let isPttEnd = false;

        // Determine PTT action based on message format
        if (json.type === "start_greeting") {
          // start_greeting message = ONLY enable PTT (call start_turn RPC)
          console.log(`🎤 [START-GREETING] Processing start_greeting - enabling PTT for device: ${this.deviceId}`);
          isPttStart = true;
        ...
        if (isPttStart) {
          ...
          console.log(`🎤 [PTT] Agent identity: ${agentParticipant.identity}`);
          console.log(`🎤 [PTT] About to call performRpc for start_turn...`);
          ...
          try {
            ...
            console.log(`✅ [PTT] start_turn RPC completed successfully: ${result}`);
          } catch (rpcError) {
            console.error(`❌ [PTT] start_turn RPC failed:`, rpcError);
            console.error(`❌ [PTT] Error details:`, {
            ...
            console.warn(`⚠️ [PTT] Agent may not support start_turn RPC - check agent implementation`);
            console.warn(`💡 [HINT] The agent needs to register this RPC handler in its entrypoint()`);
            // Continue despite error - agent might not have this RPC handler yet
          }
          ...
          console.log(`🎤 [PTT] Agent identity: ${agentParticipant.identity}`);
          console.log(`🎤 [PTT] About to call performRpc for end_turn...`);
          ...
          try {
            ...
            console.log(`✅ [PTT] end_turn RPC completed successfully: ${result}`);
          } catch (rpcError) {
            console.error(`❌ [PTT] end_turn RPC failed:`, rpcError);
            console.error(`❌ [PTT] Error details:`, {
              message: rpcError.message,
              name: rpcError.name,
              code: rpcError.code,
            ...
            console.warn(`⚠️ [PTT] Agent may not support end_turn RPC - check agent implementation`);
            console.warn(`💡 [HINT] The agent needs to register this RPC handler in its entrypoint()`);
            // Continue despite error - agent might not have this RPC handler yet
          }
        } else {
  ...
  onUdpMessage(rinfo, message, payloadLength, timestamp, sequence) {
    ...
    // Pass sequence number to sendAudio for proper frame ordering
    this.bridge.sendAudio(payload, timestamp, sequence);
...
class MQTTGateway {
  constructor() {
    ...
    this.connections = new Map(); // connectionId -> VirtualMQTTConnection
    ...
    try {
      ...
      if (topic === 'internal/server-ingest') {
        ...
        if (originalPayload.type === 'hello') {
          ...
          // Handle abort messages for virtual devices
          ...
          if (deviceInfo && deviceInfo.connection) {
            ...
            console.log(`⚠️ [ABORT] No virtual connection found for device: ${deviceId}`);
          }
        } else {
