...
// MQTT fixed header packet types
const PacketType = {
  CONNECT: 1,
  CONNACK: 2,
  PUBLISH: 3,
...
class MQTTProtocol extends EventEmitter {
   ...
   */
  processBuffer() {
    // Continuously process data in buffer until no complete messages can be processed
    while (this.buffer.length > 0) {
      // Need at least 2 bytes to start parsing (1 byte fixed header + at least 1 byte remaining length)
      ...
      try {
        ...
        // If parsing error occurs, data might be incomplete, wait for more data
  ...
  encodeRemainingLength(length) {
    ...
    do {
      ...
      // If there are more bytes, set the highest bit
      if (length > 0) {
        digit |= 0x80;
      }
  ...
  parseConnect(message) {
    ...
    // Set heartbeat interval (1.5 times the keepAlive value specified by client, in seconds)
  ...
  parsePublish(message) {
    ...
    const qos = (firstByte & 0x06) >> 1; // 0x06 is binary 00000110, used for mask to extract QoS bits
    const dup = (firstByte & 0x08) !== 0; // 0x08 is binary 00001000, used for mask to extract DUP flag
    const retain = (firstByte & 0x01) !== 0; // 0x01 is binary 00000001, used for mask to extract RETAIN flag

    // Use generic method to parse remaining length
    const { value: remainingLength, bytesRead } =
      this.decodeRemainingLength(message);
