...
```
http://172.20.10.25:8003/xiaozhi/ota/
```
...
此刻，请你用浏览器打开你的ota地址，例如我的ota地址
```
http://172.20.10.25:8002/xiaozhi/ota/
```
...
```
ws://172.20.10.25:8000/xiaozhi/v1/
```
...
找到`OTA_URL`的`default`的内容，把`https://api.tenclass.net/xiaozhi/ota/`
   改成你自己的地址，例如，我的接口地址是`http://172.20.10.25:8002/xiaozhi/ota/`，就把内容改成这个。
...
config OTA_URL
    string "Default OTA URL"
    default "http://172.20.10.25:8002/xiaozhi/ota/"
    help
        The application will access this URL to check for new firmwares and server address.
```
