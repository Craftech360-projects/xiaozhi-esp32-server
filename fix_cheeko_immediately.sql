-- IMMEDIATE FIX: Apply Cheeko role to all agents
-- Run this SQL script directly on your database to apply Cheeko changes immediately

-- First, check what templates exist in the database
SELECT id, agent_name, sort FROM ai_agent_template ORDER BY sort;

-- Update the template with ID '9406648b5cc5fde1b8aa335b6f8b4f76' (湾湾小何) to Cheeko
...
Your mission: make learning irresistibly fun, always cheeky, energetic, factual, and age-appropriate.',
    ...
    chat_history_conf = 1,
    sort = 0
WHERE id = '9406648b5cc5fde1b8aa335b6f8b4f76' OR agent_name = '湾湾小何';

-- If the above doesn't work, try updating by agent_name directly
UPDATE ai_agent_template
SET
    agent_name = 'Cheeko',
    system_prompt = '<identity>
...
Your mission: make learning irresistibly fun, always cheeky, energetic, factual, and age-appropriate.',
    ...
    chat_history_conf = 1,
    sort = 0
WHERE agent_name LIKE '%湾湾小何%' OR agent_name LIKE '%小何%';

-- Make sure all other templates have higher sort values
UPDATE ai_agent_template
SET sort = sort + 1
WHERE agent_name != 'Cheeko' AND sort <= 1;

-- Apply Cheeko to ALL existing agents immediately
...
-- Verify the changes
SELECT id, agent_name, LEFT(system_prompt, 100) as prompt_preview, sort FROM ai_agent_template ORDER BY sort;
