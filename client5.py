...
class TestClient:
    def __init__(self):
        ...
        self.server_config = None  # Will store full server config from OTA
    ...
    def get_server_config_from_ota(self) -> Optional[dict]:
        """Get server configuration from OTA endpoint"""
        try:
            url = f"http://{XIAOZHI_SERVER_HOST}:{XIAOZHI_SERVER_PORT}/xiaozhi/ota/"
            ...
            logger.info(f"🔄 Getting server config from OTA endpoint: {url}")
            response = requests.post(url, headers=headers, json=data, timeout=5)
            
            if response.status_code == 200:
                config = response.json()
                websocket_url = config.get("websocket", {}).get("url", "")
                mqtt_config = config.get("mqtt_gateway", {})
                server_config = config.get("server", {})
                
                logger.info(f"✅ Got server configuration from OTA:")
                logger.info(f"   WebSocket URL: {websocket_url}")
                logger.info(f"   MQTT Gateway: {mqtt_config.get('broker', 'N/A')}:{mqtt_config.get('port', 'N/A')}")
                logger.info(f"   UDP Port: {mqtt_config.get('udp_port', 'N/A')}")
                logger.info(f"   Server IP: {server_config.get('ip', 'N/A')}")
                
                return config
            else:
                logger.error(f"❌ OTA request failed with status {response.status_code}: {response.text}")
                return None
                
        except Exception as e:
            logger.error(f"❌ Failed to get server config from OTA: {e}")
            return None

    def setup_mqtt_connection(self) -> bool:
        """Setup MQTT connection to the gateway and get server config from OTA."""
        logger.info(f"▶️ STEP 1: Getting server configuration from OTA endpoint")
        
        # Get server configuration from OTA endpoint
        server_config = self.get_server_config_from_ota()
        if server_config:
            self.websocket_url = server_config.get("websocket", {}).get("url", "")
            
            # Update MQTT configuration from OTA if available
            mqtt_config = server_config.get("mqtt_gateway", {})
            if mqtt_config.get("enabled", False):
                # Update global MQTT configuration from OTA
                global MQTT_BROKER_HOST, MQTT_BROKER_PORT, UDP_PORT
                MQTT_BROKER_HOST = mqtt_config.get("broker", MQTT_BROKER_HOST)
                MQTT_BROKER_PORT = mqtt_config.get("port", MQTT_BROKER_PORT)
                UDP_PORT = mqtt_config.get("udp_port", UDP_PORT)
                
                logger.info(f"✅ Updated MQTT config from OTA: {MQTT_BROKER_HOST}:{MQTT_BROKER_PORT}")
                logger.info(f"✅ Updated UDP port from OTA: {UDP_PORT}")
            
            logger.info(f"✅ Server configuration obtained from OTA")
        else:
            logger.warning("⚠️ Failed to get config from OTA, using fallback configuration")
            self.websocket_url = f"ws://{XIAOZHI_SERVER_HOST}:{XIAOZHI_SERVER_PORT}/xiaozhi/v1/"
        
        logger.info(f"🔗 WebSocket URL from server: {self.websocket_url}")
        logger.info(f"▶️ STEP 1b: Setting up MQTT connection to gateway at {MQTT_BROKER_HOST}:{MQTT_BROKER_PORT}")
        return True
